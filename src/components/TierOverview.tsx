...
export const TierOverview: React.FC<TierOverviewProps> = ({
  ...
  return (
    <section className="bg-white rounded-2xl border border-gray-200/90 shadow-xs p-5 sm:p-7" id="tier-overview-section">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 relative">
        {TIERS.map((tier, index) => {
          ...
          const isUpcoming = index > TIERS.findIndex((t) => t.id === currentTier);
      ...
      <div className="mt-5 pt-4 border-t border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs sm:text-sm">
        ...
        <div className="flex items-center gap-1.5 text-gray-400 text-[11px]">
          ...
          <span>Tier thresholds shown are sample demonstration values.</span>
        </div>
      </div>
    </section>
  );
